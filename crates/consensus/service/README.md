...
`QueuedEngineRpcClient` implements the `EngineRpcClient` trait by sending `EngineRpcRequest` messages to the dedicated engine RPC queue and awaiting oneshot responses. Public RPC admission is bounded and fails fast with `ServerIsBusy` when the queue is full. The `EngineRpcProcessor` handles accepted queries concurrently behind the semaphore.
